...
namespace Aqua_Sharp_Backend.Services
{
    public class ConfigService : IConfigService
    {
        ...
        {
            var config = _context.Config.FirstOrDefault(u => u.Id == 1);
            ...
            var claims = new List<Claim>()
                {
                new Claim(ClaimTypes.NameIdentifier, config.Id.ToString()),
                new Claim("AquariumsIds", aquariums.Select(a=>a.Id).ToString()),
                new Claim("DevicesIds", devices.Select(a=>a.Id).ToString())
            };
